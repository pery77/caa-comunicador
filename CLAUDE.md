...
- **Botones extra:** `BOTONES_HABLAR = {10}` (HABLAR directo, de memoria del usuario — ⚠️ pendiente de validar en la tablet; el log `[entrada] mando: botón N` / `[entrada] tecla: …` sigue activo por consola para verificarlo con USB + `chrome://inspect`; retirarlo cuando esté confirmado). `BOTONES_ESPACIO` queda vacío: no hay botón físico libre. **Uno de los botones del mando es el selector de modo del firmware** (juego/ratón/música): cambia de modo sí o sí, la web no puede bloquearlo ni aprovecharlo — el usuario debe evitarlo y volver a pulsarlo si activa el ratón por accidente. Helper `pedirAccionGlobal(accion)` conectado para futuros atajos.
