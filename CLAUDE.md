...
- 🔜 **Dos botones extra pendientes de mapear** (HABLAR directo y ESPACIO): `index.html` ya loguea toda la entrada por consola (`[entrada] mando: botón N` y `[entrada] tecla: …`, visible con USB + `chrome://inspect`) y tiene los grupos vacíos `BOTONES_HABLAR` / `BOTONES_ESPACIO` listos para rellenar con los índices que reporte el usuario desde Android (helper `pedirAccionGlobal(accion)` ya conectado). Cuando estén mapeados, retirar o comentar los `console.log`.
